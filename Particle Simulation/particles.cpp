...
main(int argc, char** argv){
  ...
  int prev;					// Previous rank in the ring
  ...
  if(argc == 3){
    ...
    // Distributing particles among processors
  ...
  prev = (myRank - 1 + p) % p;
  total_steps = (p - 1) / 2;
  home = (myRank - total_steps + p) % p;
  recv_locals = (myRank + total_steps) % p;
  for(i = 0; i < total_steps; i++){
    ...
    MPI_Recv(remotes, number * 5, MPI_FLOAT, prev, tag, MPI_COMM_WORLD, &status);
    compute_interaction(locals, remotes, number);
  }
  MPI_Send(remotes, number * 5, MPI_FLOAT, home, tag, MPI_COMM_WORLD);
  MPI_Recv(remotes, number * 5, MPI_FLOAT, recv_locals, tag, MPI_COMM_WORLD, &status);
...
// Reads particle information from a text file - taken from the parallel version
int read_file(struct Particle *set, int size, char *file_name){
